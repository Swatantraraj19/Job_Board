  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col items-center justify-start p-4 gap-6 transition-colors duration-200">
      ...
      <div className="w-full max-w-4xl flex justify-between items-center bg-white dark:bg-zinc-900 p-4 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-800 transition-colors duration-200">
        <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400 font-bold text-xl cursor-pointer" onClick={() => navigate('/')}>
          <Briefcase />
          <span>JobBoard</span>
        </div>
        <div className="flex items-center gap-4">
          ...
          {user ? (
            <div className="flex items-center gap-4">
              <span className="text-zinc-600 dark:text-zinc-300 hidden md:inline">Welcome, <b>{user.name}</b></span>
              <button
              ...
              <button onClick={() => { logout(), navigate('/', { replace: true }) }} className="px-3 py-1 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-zinc-200 rounded-lg text-sm font-medium text-zinc-700 transition">
                Logout
              </button>
            </div>
          ) : (
            <button onClick={() => navigate('/login')} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition">
              Login
            </button>
          )}
        </div>
